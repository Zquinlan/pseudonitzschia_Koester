## Script for analyzing Pseudo-nitzschia culturing dataset
## Written by Zach Quinlan and Irina Koester 

##ABBREVIATIONS
# otu = ASVs of 16S data
# quant = quantitative feature table (feature Ids and all their intensities across samples)
# cat = categories (number of atoms, elemtal ratios, DBE, mz, RT, CANOPUS compound classes)
# feature = referes to MS data on feature based

# org = organisms, differences between Pn cultures/ species
# dom = diferences between dissolved and whole metabolome
# toxin = differences between toxic and non-toxic cultures
...
spectral_libs <- (read_csv("./Raw/Pn_GNPS_combined_Inchikeys.csv")%>%
...
quant_ra <- quant_stats_prep%>%
  select(-c(asin, xic,  chl_norm, chl))%>%
  # group_by(feature_number)%>%
  # filter(sum(.$asin) != 0)%>%
  separate(sample_code, c("Experiment", "Organism", 
                          "biological_replicates", "DOM_fil", 
...
quant_dom_permanova_exp2 <- quant_permanova%>%
  adonis(.[8:ncol(.)] ~ DOM_fil, ., perm = 1000, method = "bray", na.rm = TRUE)

quant_toxin_permanova_exp2
quant_organism_permanova_exp2
quant_dom_permanova_exp2

quant_permanova_DOMonly <- quant_stats%>%
  filter(Experiment == 'Exp2')%>%
  filter(DOM_fil == 'DOM')%>%
  mutate(toxin = case_when(Organism %like% '%sub%' ~ 'toxic',
                           Organism %like% '%mul%' ~ 'toxic',
...
quant_organism_DOM_permanova_exp2 <- quant_permanova%>%
  adonis(.[8:ncol(.)] ~ Organism, ., perm = 1000, method = "bray", na.rm = TRUE)

quant_organism_DOM_permanova_exp2

quant_permanova_unfilonly <- quant_stats%>%
  filter(Experiment == 'Exp2')%>%
...
quant_organism_unfil_permanova_exp2 <- quant_permanova%>%
  adonis(.[8:ncol(.)] ~ Organism, ., perm = 1000, method = "bray", na.rm = TRUE)

quant_organism_unfil_permanova_exp2

# STATS PERMANOVA - ASV  ---------------------------------------------------
  
otu_permanova_Exp2 <- otu_stats%>%
...
otu_organism_permanova_exp2 <- otu_permanova_Exp2%>%
  adonis(.[8:ncol(.)] ~ Organism, ., perm = 1000, method = "bray", na.rm = TRUE)

otu_organism_permanova_exp2


otu_toxin_permanova_exp2 <- otu_permanova_Exp2%>%
  adonis(.[8:ncol(.)] ~ toxin, ., perm = 1000, method = "bray", na.rm = TRUE)

otu_toxin_permanova_exp2



otu_permanova_bothExp <- otu_stats_bothExp%>%
  separate(sample_code_16S, c("Experiment", "Organism", 
                              "biological_replicates", "technical_replicates"), sep = "_")%>%
...
otu_organism_permanova_bothExp <- otu_permanova_bothExp%>%
  adonis(.[8:ncol(.)] ~ Organism, ., perm = 1000, method = "bray", na.rm = TRUE)

otu_organism_permanova_bothExp
...
write.csv(quant_binary_dom,"./Analyzed/quant_binary_dom.csv")

# PRE MATRIX QUANT SUM --  Org and Unfil--------------------------------------------
quant_sums_org <- quant_binary_org%>%
  as.data.frame()%>%
...
boxplot_quant_sums_org <- quant_sums_org%>%
  separate(sample_code, c("Experiment", "Organism", "biological_replicates", "DOM_fil", 
                          "technical_replicates"), sep = "_")%>%
  ggplot(aes(Organism, val)) + geom_boxplot()

plot(boxplot_quant_sums_org)

write_csv(quant_sums_org, "Analyzed/quant_sums_org.csv")


quant_sums_dom <- quant_binary_dom%>%
  as.data.frame()%>%
  rownames_to_column(var = "sample_code")%>%
  gather(feature_number, val, 2:ncol(.))%>%
  group_by(sample_code)%>%
...
boxplot_quant_sums_dom <- quant_sums_dom%>%
  separate(sample_code, c("Experiment", "Organism", "biological_replicates", "DOM_fil", 
                          "technical_replicates"), sep = "_")%>%
  ggplot(aes(DOM_fil, val)) + geom_boxplot()

plot(boxplot_quant_sums_dom)
...
matrix_multiplied_org <- quant_binary_org%*%cat_clean_org%>%
  ...
  left_join(quant_sums_org, by = 'sample_code')%>%
  mutate(means = mult/val,
         asin = asin(sqrt(means)))%>%
  select(-c(mult, val, means))%>%
  spread(category, asin)
...
matrix_multiplied_dom <- quant_binary_dom%*%cat_clean_dom%>%
  ...
  rownames_to_column(var = "sample_code")%>%
  gather(category, mult, 2:ncol(.))%>%
  left_join(quant_sums_dom, by = 'sample_code')%>%
  ungroup()%>%
  mutate(means = mult/val,
         asin = asin(sqrt(means)))%>%
  select(-c(mult, val, means))%>%
...
write_csv(matrix_multiplied_dom, "Analyzed/matrix_multiplied_dom.csv")
...
otu_hc <- otu_stats%>%
  ...
  select(-Experiment, -technical_replicate)
