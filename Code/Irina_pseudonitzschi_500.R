...
important_quant_org_500 <- (rf_quant_org_mda%>%
                              mutate(feature = gsub("X", "", feature))%>%
                              top_n(500, MeanDecreaseAccuracy))$feature%>%
  as.vector
...
important_quant_dom_500 <- (rf_quant_dom_mda%>%
                              mutate(feature = gsub("X", "", feature))%>%
                              top_n(500, MeanDecreaseAccuracy))$feature%>%
  as.vector()
...
write_csv(feature_info_test, "Analyzed/Ttest_elements_org_500.csv")
...
# POST-STATS -- binary for nitrogen comparison
exportN <- feature_info%>%
  gather(variable, response, 2:ncol(.))%>%
  mutate(importance = case_when(feature_number %in% important_quant_org_500 ~ "important",
                                TRUE ~ "not"),
         importance = as.factor(importance))%>%
  group_by(variable)

write.csv(exportN, "Analyzed/important_500.csv")  
...
write.csv(quant_binary_org, "Analyzed/quant_binary_org_0.01.csv") 

quant_RA_org <- quant_df%>%
  gather(sample_code, xic, 2:ncol(.))%>%
  separate(sample_code, c("Experiment", "Organism", 
                          "biological_replicates", "DOM_fil", 
                          "technical_replicates"), sep = "_", remove = FALSE)%>%
  ...
  mutate(ra = xic/sum(xic),
         chl_norm = ra/chl,
         asin = asin(sqrt(chl_norm)))%>%
  select(feature_number,chl_norm)%>%
  spread(feature_number, chl_norm)


write.csv(quant_RA_org, "Analyzed/quant_RA_org.csv") 

# POST-STATS -- MINI Quant Table organism ---------------------------------------------------


mini_quant_org <-quant_stats%>%
  filter(feature_number %in% important_quant_org_500)%>%
  spread(feature_number, asin)

write_csv(mini_quant_org, "Analyzed/mini_quant_org_500.csv")  


# POST-STATS -- Mini Quant Table unfil ------------------------------------------

mini_quant_dom <- quant_stats%>%
  filter(feature_number %in% important_quant_dom_500)%>%
  spread(feature_number, asin)

write_csv(mini_quant_dom, "Analyzed/mini_quant_dom_500.csv")  
...
cat_clean_org2 <- subset(cat_clean_org, rownames(cat_clean_org) %in% important_canopus_org)

test <- canopus_available_features_org %in% important_canopus_org
...
quant_binary_org <- quant_stats%>%  ## Okay so here we are first making the data "tidy"
  ...
  unite(feature, c("Experiment", "Organism", "biological_replicates", "DOM_fil", 
                   "technical_replicates"), sep = "_")%>%
  group_by(feature)%>%
  mutate(binary = case_when(asin > 0.05*max(asin) ~ 1,
                            TRUE ~ 0))%>%
  ungroup()%>%
  select(-asin)%>%
  spread(feature_number, binary)%>%
...
write.csv(quant_binary_org, "Analyzed/quant_binary_org_500_0.05.csv") 

# PRE-MATRIX QUANT AND CAT -- DOM_Fil ---------------------------------------------
cat_clean_dom <- cat_stats%>%
  filter(FeatureID %in% important_quant_dom_500)%>%
  column_to_rownames("FeatureID")%>%
  data.matrix(rownames.force = NA)
...
write.csv(quant_binary_dom, "Analyzed/quant_binary_dom_500_0.05.csv") 
